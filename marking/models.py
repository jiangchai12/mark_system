...
class Mark(models.Model):
    ...
    markdate = models.CharField(u'评分月份',max_length=32, blank=True, null=True)
    name = models.ForeignKey('UserProfile',blank=True, null=True, verbose_name="姓名", on_delete=models.SET_NULL)
    class Meto:
        unique_together = 'name'
    def __str__(self):
        return '<%s-%s-%s>' % (self.name.email,self.markdate, self.score)
##
class MarkDate(models.Model):
    is_mark = models.BooleanField(default=False)
    name = models.ForeignKey('UserProfile',blank=True, null=True, verbose_name="评分人", on_delete=models.SET_NULL)
    markdate = models.CharField(u'评分月份',max_length=32, blank=True, null=True)
    #mark = models.ManyToManyField('Mark', blank=True)
    def __str__(self):
        return "{'id':'%s','email':'%s','is_mark':'%s'}" % (str(self.id),self.name.email,self.is_mark)
